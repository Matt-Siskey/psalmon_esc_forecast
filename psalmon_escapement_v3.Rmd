...
`r if(params$sp=="Chinook" & params$system=="Puyallup River"){"   * Chinook     + Puyallup River       * Boise Creek       * Canyon Falls       * Clear Creek       * Fennel Creek       * Kapowsin Creek       * Salmon Creek + Trib       * South Prairie Creek       * Wilkeson Creek     + White River "}`

`r if(params$sp=="Coho" & params$system=="Puyallup River"){"   * Coho     + Puyallup River       * Canyon Falls       * Coal Mine       * Fennel Creek (lower section)       * Fiske Creek       * Spiketon "}`

`r if(params$sp=="Coho" & params$system=="East Kitsap"){"   * Coho     + East Kitsap       * Salmonberry      (15.0188; WDFW;   RM 1.6-2.1)       * Blackjack        (15.0203; Tribes; RM 5.2-6.3)       * Little Blackjack (15.0206; WDFW;   RM 0.4-1.0)       * Chico-Wildcat    (15.0229; Tribes; RM 2.6-5.0)       * Lost             (15.0234; Tribes; RM 0.0-1.9) "}`

`r if(params$sp=="Coho" & params$system=="South Sound"){"   * Coho     + South Sound       * Woodland       * Perry       * Skookum + Trib       * Rocky       * Goldsboro       * Cranberry       * Sherwood       * Coulter       * Rocky "}`

`r if(params$sp=="Steelhead"){"   * Steelhead     + Puyallup River "}`
...
if(params$sp=="Chinook"){
  ...
  live_counts_fall <-dplyr::filter(live_counts, Date > start_date & Date < end_date)
}
...
live_counts_fall$StreamCatalogCode <-specify_decimal(as.numeric(live_counts_fall$StreamCatalogCode),4)
...
live_sums_trim      <-live_sums[order(live_sums$System, live_sums$Date),] # reorder dataset so dates are in order
...
for(d in 1:length(fix_survey)){
  ...
  # fixes_temp <-live_sums_sort2[which(live_sums_sort2$System!=fix_survey[d]),]
  fixes[[d]] <-dat
}
live_sums_fixed <-rbind(map_df(fixes, ~as.data.frame(.)), dplyr::filter(live_sums_sort2, !(System %in% fix_survey)))
...
colnames(system_AUC)         <-c("system.id","AUC_sum")
...
# Included
live_sums_sort3$System <-specify_decimal(as.numeric(live_sums_sort3$System),4)
...
if(params$sp=="Chinook"){
  ...
  if(check_odd_even(curr_yr)=="Even"){
    ...
    live_counts_fall$StreamCatalogCode <-specify_decimal(as.numeric(live_counts_fall$StreamCatalogCode),4)
    Redds           <-live_counts_fall[,c("Date","StreamCatalogCode","Redds")]
    ...
    redd_esc$system.id <-specify_decimal(as.numeric(redd_esc$system.id),4)
    
    ##### Join new (system_AUC calc above) and old (read in csv) calcs
    Redd_AUC_new                <-merge(system_AUC,redd_esc,by="system.id")
    col_order                   <-c("year", "system.id", "system.name","redd_based_esc","AUC_sum")
    Redd_AUC_new                <-Redd_AUC_new[, col_order]
    Redd_AUC_new$AUC_fishdays   <-Redd_AUC_new$AUC/fish_days
    Redd_AUC_new$Redd_AUC_ratio <-as.numeric(Redd_AUC_new$redd_based_esc)/Redd_AUC_new$AUC_fishdays
    Redd_AUC_curryr             <-Redd_AUC_new[,c("year","system.id","system.name","redd_based_esc","AUC_sum","AUC_fishdays")]
    Redd_AUC_curryr$AUC_adj     <-rep(NA,nrow(Redd_AUC_curryr))
    Redd_AUC                    <-rbind(Redd_AUC,Redd_AUC_curryr[,c("year","system.id","system.name","redd_based_esc","AUC_fishdays","AUC_adj")])
    ...
    SPC_ReddAUC_ratio_mean <-mean(filter(even_yrs_SPC, year >= 2000)$Redd_AUC_ratio)
    ...
    system_AUC <-new_AUC[,c("system.id","AUC_fishdays","system.name")]
  }
  ...
  # if(check_odd_even(curr_yr)=="Even"){
  #   puy_tribs_redd   <-new_AUC %>% filter(system.name %in% c("Canyon Falls","Fennel Creek","Kapowsin Creek"))
  #   puy_tribs_y     <-(sum(puy_tribs_redd$redd_based_esc))
  # }
  # if(check_odd_even(curr_yr)=="Odd"){
  ...
  if(check_odd_even(curr_yr)=="Odd"){
    table_3 <-new_AUC[,c("year", "system.id", "system.name","AUC_adj")] %>%
                # dplyr::filter(.,year==max(year)) %>%
                kbl(row.names = FALSE, 
                    caption = "Table 3. The sum of area-under-the-curve (AUC) adjusted for fish days and Redd/AUC ratio.", 
                    digits = 1) %>%
...
```{r white_esc, eval=eval_white, fig.width=c(6,6), fig.height=c(2,4), fig.cap=c("**Figure 2.** Total fish passed at Mud Mountain Dam.", "**Figure 3.** Total UM, HOR, and AP fish at Mud Mountain Dam."), fig.topcaption = TRUE, include=TRUE, echo=TRUE, results='asis'}
...
if(params$sp=="Chinook"){
  ...
  misc_dat    <-read.csv(paste0(drive_sp,curr_yr,"_Puyallup River_",params$sp," Misc Data.csv"))
  ...
  if(curr_yr>=2024){
    ...
    for(i in 1:length(unsamp_weeks)){
      ...
      # wild adults
      prop_W_A <-sum(temp_df$wild_total)/sum(temp_df$wild_total, temp_df$AP_total, temp_df$hatchery_total, temp_df$Ad.Clipped)
      ...
      prop_AP_A <-sum(temp_df$AP_total)/sum(temp_df$wild_total, temp_df$AP_total, temp_df$hatchery_total, temp_df$Ad.Clipped)
      ...
      # hatchery adults
      prop_H_A <-sum(temp_df$hatchery_total)/sum(temp_df$wild_total, temp_df$AP_total, temp_df$hatchery_total, temp_df$Ad.Clipped)
      ...
      # fall adults
      prop_AD <-sum(temp_df$Ad.Clipped)/sum(temp_df$wild_total, temp_df$AP_total, temp_df$hatchery_total, temp_df$Ad.Clipped)
  ...
  if(curr_yr %in% NOR_ass$return_yr == FALSE){
    ...
    NOR_ass[nrow(NOR_ass),"fall"]            <-dplyr::filter(misc_dat, Type == "fall_NOB")$Value
  ...
  for(y in 1:length(yrs)){
    ...
    if(yrs[y]>=2024){
      counts[which(counts$return_yr==as.numeric(yrs[y])),"fall_passed"] <-(dplyr::filter(counts,return_yr==yrs[y])$fall_unk_total -
                                                                           ...
                                                                           dplyr::filter(misc_dat, Type == "fall_total")$Value
    }
  }
  ...
  white_esc           <- dplyr::filter(misc_dat, Type == "fall_total")$Value + sum(unsamp$AD_exp)
  ...
  mmd_counts_long <-tidyr::gather(counts[,c("return_yr","Stage","UM_Total","HOR_Total","AP_Total")], 
                                  Stock, Escapement, "UM_Total","HOR_Total","AP_Total")
  ...
  ### Tables
  print(NOR_ass[,c("return_yr","prop_spring_NOR","prop_spring_HOR","prop_fall","prop_unk","prop_spring_total","prop_fall_unk")] %>%
                kbl(caption="Table 3. Stock composition based on adipose fin mark status and coded wire tag interceptions at Mud Mountain Dam on the White River.", digits = 3) %>%
                kable_classic(full_width = F, position = "left", html_font = "Cambria", fixed_thead = T))
...
if(params$sp=="Chinook"){
  ...
  if(check_odd_even(curr_yr)=="Even"){
    new_esc      <-row_to_names(as.data.frame(t(Redd_AUC_new[,c("system.name","redd_based_esc")])),row_number = 1)
    new_esc_keep <-new_esc[,system.names.auc]
    esc_ts_total[nrow(esc_ts_total),c("Year",colnames(new_esc_keep),"Stock")] <-c(curr_yr,(as.numeric(new_esc_keep["redd_based_esc",])),"Total")
  }
  if(check_odd_even(curr_yr)=="Odd"){
  ...
  survey_counts_tribe$WRIA                        <-specify_decimal(as.numeric(survey_counts_tribe$WRIA),4)
  ...
  if(curr_yr >= 2023){
    spc_mark_status        <-dplyr::filter(dead_fish_wria, stream_catalog_code == "10.0429" & run_year == curr_yr)
  ...
  if(curr_yr < 2024){mark_stat[(nrow(mark_stat)+1),] <-c("White River",
                                      white_esc,
